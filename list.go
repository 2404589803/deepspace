...
func listCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "Query DeepSeek AI requests based on conditions",
			...
			for _, request := range requests {
				if verbose {
					t.AppendRow(table.Row{
						...
						request.ChatCmpl(),
						request.DeepSeekRequestID.String,
						request.DeepSeekUID.String,
						strconv.FormatInt(request.DeepSeekServerTiming.Int64, 10),
...
func inspectCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "inspect",
		Short: "Inspect the specific content of a DeepSeek AI request",
		Run: func(cmd *cobra.Command, args []string) {
	...
	flags.StringVar(&requestID, "requestid", "", "request id returned from DeepSeek AI")
...
func cleanupCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "cleanup",
		Short: "Cleanup DeepSeek AI requests",
		Run: func(cmd *cobra.Command, args []string) {
