...
func (__imp *implPersistence) createTable() error {
	...
	querycreateTable := "create table if not exists deepseek_requests ( id                     integer not null constraint deepseek_requests_pk primary key autoincrement, request_method         text    not null, request_path           text    not null, request_query          text    not null, request_content_type   text, request_id             text, deepseek_id            text, deepseek_gid           text, deepseek_uid           text, deepseek_request_id    text, deepseek_server_timing integer, response_status_code   integer, response_content_type  text, request_header         text, request_body           text, response_header        text, response_body          text, error                  text, response_ttft          integer, latency                integer, endpoint               text, created_at             text default (datetime('now', 'localtime')) not null );\r\n"
...
func (__imp *implPersistence) Persistence(requestID string, requestContentType string, requestMethod string, requestPath string, requestQuery string, deepseekID string, deepseekGID string, deepseekUID string, deepseekRequestID string, deepseekServerTiming int, responseStatusCode int, responseContentType string, requestHeader string, requestBody string, responseHeader string, responseBody string, programError string, responseTTFT int, createdAt string, latency time.Duration, endpoint string) (int64, error) {
	...
	if errPersistence = sqlTmplPersistence.Execute(sqlPersistence, map[string]any{
		...
		"deepseekGID":          deepseekGID,
		"deepseekUID":          deepseekUID,
		"deepseekRequestID":    deepseekRequestID,
		"deepseekServerTiming": deepseekServerTiming,
